...
describe("FossaTokenTest", function () {
  ...
  it("Test sprawdza funkcję transfer", async () => {
    ...
    await FossaTokenContract.connect(owner).approve(
      buyer.address,
      amountApprovwed
    );
    ...
    const balanceBeforeBuyer = await FossaTokenContract.balanceOf(
      buyer.address
    );
  ...
  it("Test sprawdza funkcje approve i transferFrom ", async () => {
    ...
    const balanceOwnerBefore = await FossaTokenContract.balanceOf(
      owner.address
    );
    const balanceReceiverBefore = await FossaTokenContract.balanceOf(
      reciver.address
    );

    await FossaTokenContract.connect(owner).approve(spender.address, amount);
    const allowanceBefore = await FossaTokenContract.allowance(
      owner.address,
      spender.address
    );
    ...
    try {
      await FossaTokenContract.connect(spender).transferFrom(
        owner.address,
        reciver.address,
        9999
      );
    ...
    const allowanceAfter = await FossaTokenContract.allowance(
      owner.address,
      spender.address
    );
    const balanceOwnerAfter = await FossaTokenContract.balanceOf(owner.address);
    const balanceReceiverAfter = await FossaTokenContract.balanceOf(
      reciver.address
    );
